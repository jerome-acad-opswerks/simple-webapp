...
def validate_html(file_path):
    ...
    try:
        ...
        pytest.fail(f"HTML syntax error in file {file_path}: {str(e)}", pytrace=True)
        ...
        with open(file_path, "r", encoding="utf-8") as f:
            ...
            for hex_color in matches:
                if not re.match(r"^#[0-9a-fA-F]{6}$", hex_color):
                    pytest.fail(f"Invalid HEX color in {file_path}: {hex_color}", pytrace=True)
    except Exception as e:
        pytest.fail(f"Error checking HEX colors in {file_path}: {str(e)}", pytrace=True)
